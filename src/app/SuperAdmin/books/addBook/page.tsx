...
import { Pencil, User, ImagePlus, CircleX, Save, BookOpen, School, Calendar, BookText } from "lucide-react"
...
const AgregarLibro = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/books/create`, {
        method: "POST",
        body: formData,
      })
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-gray-50 to-gray-100 p-6">
      <div className="bg-white shadow-xl rounded-2xl overflow-hidden w-full max-w-5xl flex flex-col md:flex-row">
        ...
        <div className="flex flex-col items-center justify-between w-full md:w-2/5 bg-gradient-to-br from-[#03045e] to-[#0077b6] p-8 text-white">
          <div className="w-full">
            <h2 className="text-3xl font-bold mb-6 text-center">Portada del Libro</h2>
            ...
            <div className="bg-white/20 backdrop-blur-sm p-4 rounded-xl">
              <label className="flex flex-col items-center justify-center w-full p-3 border-2 border-white/50 border-dashed rounded-lg cursor-pointer hover:bg-white/10 transition-all">
                <div className="flex flex-col items-center justify-center">
                  ...
                  <p className="text-sm font-medium">{archivo ? archivo.name : "Seleccionar PDF"}</p>
                </div>
                <input type="file" accept="application/pdf" onChange={handleArchivoChange} className="hidden" />
              </label>

              {previewPdf && (
                <div className="mt-4 bg-white rounded-lg overflow-hidden shadow-lg">
                  <iframe src={previewPdf} className="w-full h-40" title="PDF preview" />
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex w-full mt-8 gap-4 justify-center">
            ...
            <button
              className="bg-white text-[#03045e] px-5 py-2.5 rounded-lg flex items-center gap-2 justify-center font-medium hover:bg-gray-100 transition-all duration-300 shadow-lg"
        ...
        <div className="flex-1 p-8">
          <h2 className="text-3xl font-bold mb-8 text-gray-800 border-b pb-4">Agregar Nuevo Libro</h2>

          <div className="space-y-6">
            <div>
              <label className="block text-gray-700 font-medium mb-2">Nombre del Libro</label>
              <div className="relative">
                ...
                />
                <BookText className="absolute left-4 top-3.5 text-[#03045e]" size={20} />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                />
                <User className="absolute left-4 top-3.5 text-[#03045e]" size={20} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <School className="absolute left-4 top-3.5 text-[#03045e]" size={20} />
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    className={`w-full p-3 border border-gray-300 rounded-lg pl-12 text-gray-700 appearance-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all bg-white ${!fkIdArea ? "opacity-60 cursor-not-allowed" : ""}`}
                    value={fkIdSemestre ?? ""}
                    onChange={(e) => {
                      const selected = Number.parseInt(e.target.value)
                  ...
                  <Calendar className="absolute left-4 top-3.5 text-[#03045e]" size={20} />
                  <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                    <svg className="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                      <path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z" />
                    </svg>
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    className={`w-full p-3 border border-gray-300 rounded-lg pl-12 text-gray-700 appearance-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all bg-white ${!fkIdSemestre ? "opacity-60 cursor-not-allowed" : ""}`}
                    value={fkIdMateria ?? ""}
                    onChange={(e) => setFkIdMateria(Number.parseInt(e.target.value))}
                  ...
                  <Pencil className="absolute left-4 top-3.5 text-[#03045e]" size={20} />
                  <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                    <svg className="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                      <path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z" />
                    </svg>
